...
#All "foods" per allergen
allergydict = {}
for row in rows:
    splitted = row.split("(contains ")
    ingredientList = set(splitted[0].split(" ")[:-1])
    allergensList = splitted[1].replace(")","").replace(" ", "").split(",")
    for allergen in allergensList:
        if allergen not in allergydict.keys():
            allergydict[allergen] = []
        if ingredientList not in allergydict[allergen]:
            allergydict[allergen].append(ingredientList)

    ings = ings | set(ingredientList) #All ingredients
    ingsList = ingsList + list(ingredientList) #All ingredients including repetitions

#Ingreedients that can contain an allergen (present in all foods with the alergen)
canContainAllergen = set()
for allergen, foods in allergydict.items():
    p = foods[0]
    for food in foods[1:]:
        p = p & food
    canContainAllergen = canContainAllergen | p
    allergydict[allergen] = p #What alergen can be present in what ingredients (for part 2)

safe = ings - canContainAllergen

countsum = 0
for item in safe:
    countsum += ingsList.count(item)
print(countsum)

#Part 2
pairs = []
allergensList = list(map(list, allergydict.items()))

def listSort(tuple):
    return len(tuple[1])

length = len(allergensList)
while len(pairs) < length:
    allergensList.sort(key=listSort) #Sort and takeout first item (length 1)
    pair = allergensList[0]
    del allergensList[0]

    item = pair[1].pop() #Pop the only item and add it to final pairs (allergen, item)
    pairs.append( (pair[0], item) )

    for i, pair in enumerate(allergensList):
        allergensList[i][1] = pair[1] - {item} #Remove from all others

#Sort on first string
def allergenSort(tuple):
    return tuple[0]
pairs.sort(key = allergenSort)
